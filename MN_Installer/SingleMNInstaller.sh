...
case $key in
    ...
    -h|--help)
    cat << EOL
SPRO Masternode installer arguments:
...
# Set these to change the version of SPRO to install
TARBALLURL="https://github.com/SproutsOrigins/Sprouts-Origins/releases/download/1.0.0/SPRO_ubuntu_1804.tar.gz"
TARBALLNAME="SPRO_ubuntu_1804.tar.gz"
#BOOTSTRAPURL="#"
...
echo "
 ...
 | the SPRO Team CANNOT assist you in repairing  |::
...
echo "" && echo 'Added user "SPRO"' && echo ""
sleep 1
...
rm $TARBALLNAME
mv ./sproutsoriginsd /usr/local/bin
mv ./sproutsorigins-cli /usr/local/bin
mv ./sproutsorigins-tx /usr/local/bin
rm -rf $TARBALLNAME

# Create .sproutsorigins directory
mkdir $USERHOME/.sproutsoriginsd
...
# Create sproutsorigins.conf
touch $USERHOME/.sproutsorigins/sproutsorigins.conf
cat > $USERHOME/.sproutsorigins/sproutsorigins.conf << EOL
${INSTALLERUSED}
rpcuser=${RPCUSER}
...
masternode=1
addnode=155.138.218.214:6942
seednode=216.21.165.61:6942
EOL
chmod 0600 $USERHOME/.sproutsorigins/sproutsorigins.conf
chown -R $USER:$USER $USERHOME/.sproutsorigins

sleep 1

cat > /etc/systemd/system/sproutsorigins.service << EOL
[Unit]
...
ExecStart=/usr/local/bin/sproutsoriginsd -conf=${USERHOME}/.sproutsorigins/sproutsorigins.conf -datadir=${USERHOME}/.sproutsorigins
ExecStop=/usr/local/bin/sproutsorigins-cli -conf=${USERHOME}/.sproutsorigins/sproutsorigins.conf -datadir=${USERHOME}/.sproutsorigins stop
Restart=on-abort
[Install]
WantedBy=multi-user.target
EOL
sudo systemctl enable sproutsorigins.service
sudo systemctl start sproutsorigins.service
